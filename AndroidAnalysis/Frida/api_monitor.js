...
function hook(api, callback) {
    ...
    try {
        ...
        var name = api.name;
        try {
            // Checking if android version of emulator is compatible with app's android version
            if (api.target && parseInt(Java.androidVersion, 10) < api.target) {
                return
            }
            functionSignature = Java.use(klass)[method];
            if (!functionSignature) {
                send('[API Monitor] Cannot find ' + klass + '.' + method);
                return
            }
        } catch (err) {
            send('[API Monitor] Cannot find ' + klass + '.' + method);
        ...
        var overloadCount = functionSignature.overloads.length;
        for (var i = 0; i < overloadCount; i++) {
            functionSignature.overloads[i].implementation = function () {
                var context = this
                var funcArgs = [].slice.call(arguments);
                // Calling original function
                var retval = context[method].apply(context, funcArgs);
                if (callback) {
                    var calledFrom = Exception.$new().getStackTrace().toString().split(',').slice(1);
                    var message = {
                        name: name,
                        class: klass,
                        method: method,
        ...
        send('[API Monitor] - ERROR: ' + klass + "." + method + " [\"Error\"] => " + err);
    }
}
